...
@router.get("/default/guest")
async def get_guest_problem(
    db: Annotated[database_dependency, Depends()],
    user: Annotated[User, Depends(get_current_user)],
) -> ProblemFormat:
    """Get the first available problem for a guest user.
    ...
    for problem in problems:
        ...
        if temp_problem.name == "Forest problem":
            fid = counter
        counter += 1

    return all_problems[fid]
